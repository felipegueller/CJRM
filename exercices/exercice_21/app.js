...
const isLessThan501 = number => number < 501
const countNumberLessThan501 = (acc, item) =>
    isLessThan501(item) ? ++acc : acc
// const quantityNumberLessThan501 = crazyNumbers.filter(isLessThan501).length
const quantityNumberLessThan501 = crazyNumbers.reduce(countNumberLessThan501, 0)

console.log({quantityNumberLessThan501})
...
const squareNumbers = numbers.map(getSquareNumbers)
...
/*
  ...
  - Nome 3
*/
const getFormatedList = (acc, { name }) => `${acc}- ${name}\n`
const gameNames = cart.reduce(getFormatedList, '')
...
const getTaratinoMoviesReleasedBefore2000 = movie => {
    const { release } = movie
    const isReleasedBefore2000 = release < 2000;

    return isReleasedBefore2000
}
const moviesReleasedBefore2000 = tarantinoMovies.filter(getTaratinoMoviesReleasedBefore2000)
...
const tvShowsNames = tvShows.map(getNames)
